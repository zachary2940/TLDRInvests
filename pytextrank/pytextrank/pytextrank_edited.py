...
#{"graf": [[3, "SEE", "see", "VB", 1, 430], [0, "ALSO", "also", "RB", 0, 431], [0, ":", ":", ".", 0, 432]], "id": 1, "sha1": "b915b63d4cd027cc84dc62cdfd51358a5f82a048"}
...
def parse_graf (doc_id, graf_text, base_idx, spacy_nlp=None):
    ...
    doc = spacy_nlp(graf_text) #removed extra argument parse=True
...
def build_graph (json_iter):
    ...
    for meta in json_iter:
        ...
        for pair in get_tiles(map(WordNode._make, meta["graf"])):
            ...
            try:
                # deprecated: graph.edge[pair[0]][pair[1]]["weight"] += 1.0
...
def enumerate_chunks (phrase, spacy_nlp):
    ...
    if (len(phrase) > 1):
        ...
        doc = spacy_nlp(text.strip()) # deprecated , parse=True
...
def normalize_key_phrases (path, ranks, stopwords=None, spacy_nlp=None, skip_ner=True):
    ...
    for rl in sorted(single_lex["lex"].values(), key=lambda rl: rl.rank, reverse=True):
        ...
        yield rl # i need id to appear here
